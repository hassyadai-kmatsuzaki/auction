    ...
    <Box>
      ...
      <Paper sx={{ p: 3, mb: 3 }}>
        ...
        <Typography variant="body2" color="text.secondary" gutterBottom>
          1行=1オークション。期間内のオークションについて、出品/落札/入札者数（実際に入札した人数）/会場参加者数（会場ボタンを押した人数）の件数・会員数（全体スナップショット）に加え、落札金額・落札者/出品者手数料・送料（税抜）、消費税3区分（落札者・インボイス有出品者・インボイス無出品者）、落札者請求合計・出品者支払合計（税込）を出力します。消費税は請求書・支払通知書と同じ単位で丸めるため帳票合計と一致します。末尾に行動分析（閲覧数・閲覧UU・お気に入り数・指値数）を付与します（テストユーザーは除外、行動分析ダッシュボードと一致）。
        </Typography>
        <Divider sx={{ my: 2 }} />
      ...
      <Paper sx={{ p: 3 }}>
        ...
        <Typography variant="body2" color="text.secondary" gutterBottom>
          全出品（未落札含む）を出品単位で出力（出品番号・出品ID付き。出品IDはレーン割当前は「未発行」）。オークションを選択するか、期間で絞り込めます。落札済みの行のみ金額列（落札金額・落札者/出品者手数料・送料・消費税3区分・落札者請求合計・出品者支払合計）が入り、消費税はインボイス有/無の該当区分のみに出力します。消費税は生体単位で丸めるため、帳票単位で丸めるサマリーCSVとは合計が一致しない場合があります。各生体の行動分析（閲覧数・閲覧UU・お気に入り数・指値数）も付与します（テストユーザーは除外）。
        </Typography>
        <Divider sx={{ my: 2 }} />
        <Grid container spacing={2} alignItems="center">
