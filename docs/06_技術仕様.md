...
#### 状態管理
- **Zustand** （グローバルstate管理）

#### リアルタイム通信
- **Laravel Echo** + **Laravel Reverb**（Pusherプロトコル互換）
- WebSocket接続（Redis Pub/Sub経由）
...
- **Laravel Reverb** （自前WebSocketサーバー、Pusherプロトコル互換）
- **Redis** (キャッシュ・キュー・セッション・Pub/Sub)
...
#### コンテナ構成（開発環境）
```yaml
auction-proxy   (Nginx)          … リバースプロキシ
auction-app     (PHP-FPM)        … Laravel API
auction-queue   (PHP)            … キューワーカー (countdown + default)
auction-reverb  (PHP)            … WebSocketサーバー (Laravel Reverb)
auction-redis   (Redis 7)        … Cache/Queue/Session/Pub-Sub
auction-db      (MySQL 8.0)      … 永続データ
```
...
| コンポーネント | スペック | 役割 | 月額 |
|--------------|---------|------|------|
| EC2 | t3.large (2vCPU / 8GB) | Nginx + PHP-FPM + Reverb + Queue Worker | $79 |
| RDS | db.t3.medium Multi-AZ (2vCPU / 4GB) | MySQL 8.0 (max_connections=150) | $128 |
| ElastiCache | cache.t3.medium + レプリカ (3GB) | Redis 7.x (Cache/Queue/Session/Pub-Sub) | $99 |
| ALB | Application Load Balancer | HTTPS終端、WebSocketルーティング | $30 |
| ACM | SSL証明書 | 無料・自動更新 | $0 |
| Route 53 | DNS | ALB統合 | $1 |
| CloudWatch | 監視・ログ | メトリクス、アラート | $10 |
| **合計** | | | **約 $376/月** |

> 詳細は `AWS本番環境構築ガイド.md` および `同時接続500人_負荷耐性レポート.md` を参照
