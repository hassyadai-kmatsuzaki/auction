...
#### 本番環境（500人同時接続対応）
- AWS EC2 t3.large (2vCPU / 8GB RAM)
- AWS RDS db.t3.medium Multi-AZ (MySQL 8.0, max_connections=150)
- AWS ElastiCache cache.t3.medium + レプリカ (Redis 7.x, 3GB RAM)
- AWS ALB (Application Load Balancer, WebSocket対応)
- AWS S3 (ファイルストレージ)
- AWS ACM (SSL証明書)
- AWS Route 53 (DNS)
- AWS CloudWatch (監視・ログ)

> 詳細は `AWS本番環境構築ガイド.md` を参照
