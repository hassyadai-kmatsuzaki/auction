...
**承認者**: 代表取締役 松崎 航平  
**版数**: 1.3  
**最終更新**: 2026年6月10日（全マイグレーションと突合）  
**対象システム**: 日本メダカオンライン市場  
**基盤**: Laravel 12 / MySQL 8.0  
**テーブル総数**: 69（93マイグレーション適用後の最終形）

> 本ドキュメントは設計思想・ER 図・アーカイブ戦略を扱います。
> **全69テーブルのカラム・制約・インデックス・FK・Enum 値の完全な定義は
> [テーブル定義一覧](テーブル定義一覧.md) を参照してください**（2026-06-10 に全マイグレーションから再構成）。
...
| 2 | roles | ロール定義（admin / seller / participant / media_editor） |
...
v1.1 公開以降に追加されたテーブル群。AI 機能・課金基盤・配送精密化・監査強化を担う。
（各テーブルの全カラム定義は [テーブル定義一覧](テーブル定義一覧.md) を参照）

**決済・サブスクリプション**
| テーブル | 用途 |
|---------|------|
| plans | 年会費プランマスタ（id=1 bid_only / id=2 both 固定 2件運用） |
| subscriptions | 1ユーザー1サブスク。Square Customer ID／カード末尾4桁／状態を保持 |
| payments | 決済トランザクション（status, paid_at）。`exports/subscriptions.csv` の最終支払日に使用 |
| square_webhook_events | Square Webhook の冪等処理用イベントログ |
| seller_settlements | 出品者ネット精算（落札合計／手数料／送料合計） |
| escrow_transactions | エスクロー決済の状態管理（confirm/release/refund/dispute） |

**マーケティング・LP**
| テーブル | 用途 |
|---------|------|
| email_campaigns | 一斉メール配信のキャンペーン本体 |
| email_campaign_recipients | キャンペーン宛先・配信結果 |
| lp_cvr_settings | 買受者LP／出品者LPの流入経路別 CTA URL |

**マスタ拡張**
| テーブル | 用途 |
|---------|------|
| species_types | 種別マスタ（メダカ／水草／金魚など）。`bag_specs`・`box_capacities` を種別配下に持つ |
| bag_mix_restrictions | 種別×混載制約 |
| pedigree_certificates | 血統証明書（出品単位） |
| auction_seller_shipments | 出品者別の発送パッケージ（伝票登録の親） |

**AI／不正検知**
| テーブル | 用途 |
|---------|------|
| ai_image_analyses | 画像解析結果（個体特徴・品質スコア） |
| ai_price_predictions | 価格予測結果 |
| ai_fraud_alerts | 不正検知アラート |
| ai_recommendations | レコメンド出力 |

**その他**
| テーブル | 用途 |
|---------|------|
| audit_log_entries | 新監査ログ（旧 `audit_logs` から構造刷新） |
| saved_searches | ユーザーの検索条件保存 |
| user_reviews | 取引終了後のユーザー評価 |
| idempotency_keys | Internal API 等の冪等性制御（scope × key で重複リクエスト排除） |

> 既存テーブルへの主なカラム追加（v1.1 以降）:
> - `auctions`: `is_test`, `is_published`, `published_at`, `payment_deadline_hours`, `shipping_deadline_hours`, ソフトデリート
> - `items`: `exhibit_code` / `exhibit_code_issued_at`, `is_anonymous`, `seller_display_order`, `quantity_unit`, 親魚情報 JSON, 飼育環境 JSON, `species_type_id`
> - `won_items`: `shipping_fee`, `shipping_fee_auto`, `shipping_approved_at`, `shipping_approved_by`, `shipping_adjustment_reason`, `shipping_locked_at`
> - `users`: `trade_name`, `company_name`, `is_test`
...
> ⚠️ **本番のみの手動インデックス**: 2026-05-21 に本番 DB へ手動 ALTER で
> `idx_active_limits (item_id, is_triggered, limit_price)` を追加済み（指値判定クエリの高速化）。
> **マイグレーションには未反映**のため、DB を再構築する場合は再投入が必要。
...
| 版数 | 日付 | 改訂内容 | 作成・承認 |
|---|---|---|---|
| 1.0〜1.2 | 2026-04〜2026-05-19 | 初版作成および継続改訂（アーカイブ戦略・2026-04〜05 追加テーブル反映） | 株式会社BeerO'Clock 松崎 航平 |
| 1.3 | 2026-06-10 | 全93マイグレーションと突合（69テーブル確定）、全カラム定義をテーブル定義一覧として分冊化 | 株式会社BeerO'Clock 松崎 航平 |
